...
% Split the data into training set T (750 samples) and validation set V (250 samples)
% Note that T and V must be disjoint sets.
random_indices = randperm(n);
T_size = 750;
V_size = 250;
T_indices = random_indices(1:T_size);
V_indices = random_indices(T_size+1:T_size+V_size);
...
title('Log Joint Likelihood vs. log(sigma)');
grid on;
...
xlabel('log(sigma)');
ylabel('D Value (D)');
title('D Value vs. log(sigma)');
grid on;

% Display the best sigma and LL value and D values
fprintf('Best Sigma for Max. LL: %.4f\n', best_sigma);
fprintf('Best Log Joint Likelihood (LL): %.4f\n', best_LL);
fprintf('D Value for Best LL: %.4f\n\n', best_LL_D_value);
fprintf('Best Sigma for Min. D: %.4f\n', sigma_best_D);
fprintf('Best D Value: %.4f\n', best_D);
...
xlabel('x');
ylabel('Density');
title('Estimated Density vs. True Density (Best LL)');
...
xlabel('x');
ylabel('Density');
title('Estimated Density vs. True Density (Best/Min D)');
legend('Location', 'North');
grid on;
